...
function [LL, grad] = getLL()
    ...
    if (expVokBool == 0)
        ...
        grad = ones(Op.n,1);
        disp('The parameters not feasible')
        return; 
    end
    ...
    for i = 1:Op.n
        ...
        p = sparse(p);
        gradExpV(i).value =  sparse(A\(v + p)); % partial z/ partial beta_q
    end
    ...
    for n = 1:nbobs    
        ...
        for i = 1: Op.n
            Gradient(n,i) = - gradExpV(i).value(orig,dest - lastIndexNetworkState)/expV(orig); % (equation 17 in Fosgerau et al. 2013)
        end
